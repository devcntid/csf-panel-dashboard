...
function parseDate(dateStr: string | number): Date | null {
  ...
  const monthMap: Record<string, number> = {
    'january': 0, 'januari': 0, 'february': 1, 'februari': 1,
    'march': 2, 'maret': 2, 'april': 3, 'may': 4, 'mei': 4,
    'june': 5, 'juni': 5, 'july': 6, 'juli': 6,
    'august': 7, 'agustus': 7, 'september': 8,
    'october': 9, 'oktober': 9, 'november': 10, 'december': 11, 'desember': 11
  }
...
export async function POST(request: NextRequest) {
  try {
    ...
    const categories = await sql`SELECT name, id_program_zains FROM master_target_categories`
    const categoryMap: Record<string, string> = {}
    ;(categories as any[]).forEach((cat: any) => { categoryMap[cat.name] = cat.id_program_zains })
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        try {
          ...
          const [insertedTransaction] = (await sql`
            ...
            VALUES (
              ${clinicId}, NULL, ${polyId}, ${insuranceTypeId}, ${trxDateFormatted}, ${trxNo}, 1, ${ermNo}, ${nik || null}, ${patientName},
          ...
          const [insertedPatient] = (await sql`
            INSERT INTO patients (
              clinic_id, erm_no, nik, full_name, first_visit_at, last_visit_at, visit_count, id_donatur_zains, erm_no_for_zains
            )
            VALUES (
              ${clinicId}, ${ermNo}, ${nik || null}, ${patientName}, ${trxDateFormatted}, ${trxDateFormatted}, 1, ${defaultIdDonaturZains}, ${ermNoForZains}
            )
          ...
          if (patientId) {
            await sql`
              UPDATE transactions
              SET patient_id = ${patientId}, updated_at = NOW()
    ...
    console.error('Error batch uploading transactions:', error)
    return NextResponse.json(
      { error: 'Gagal memproses batch', details: error.message },
      { status: 500 }
    )
  }
}
