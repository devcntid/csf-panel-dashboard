...
export default function DashboardLayout({
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-slate-50">
      {/* Sidebar */}
      <aside
        className={`fixed lg:static inset-y-0 left-0 z-50 bg-teal-700 text-white transform transition-all duration-300 ease-in-out ${
          // Mobile: slide in/out
        ...
        <div className={`flex flex-col h-full transition-all duration-300 ${sidebarCollapsed ? 'lg:w-0 lg:overflow-hidden' : 'w-64'}`}>
          ...
          <div className="p-4 border-b border-teal-600">
            <div className="relative flex items-center justify-center">
              {/* Logo mobile - hanya logo putih, lebih besar, tanpa teks */}
              ...
              <Button
                variant="ghost"
                size="icon"
                className="lg:hidden text-white hover:bg-teal-600 absolute right-2 top-2"
          ...
          {/* User Info */}
          <div className={`p-4 border-b border-teal-600 transition-opacity duration-300 ${sidebarCollapsed ? 'lg:opacity-0 lg:hidden' : 'lg:opacity-100'}`}>
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-teal-200 text-xs">{userRole}</p>
              </div>
            </div>
          </div>
                      ...
                      <button
                        ...
                        className={`w-full flex items-center justify-between gap-3 px-3 py-2.5 rounded-lg text-left transition-colors ${
                          pathname.startsWith(item.href)
                            ? 'bg-teal-600 border-l-3 border-white'
      ...
      <main className={`flex-1 overflow-auto relative transition-all duration-300 ${sidebarCollapsed ? 'lg:ml-0' : ''}`}>
        ...
        <div className="sticky top-0 z-20 bg-white border-b border-slate-200 shadow-sm">
          <div className="flex items-center h-16 px-4 lg:px-6">
            ...
            <Button
              ...
              className="hidden lg:flex mr-3 bg-teal-600 hover:bg-teal-700 text-white shadow-sm hover:shadow-md transition-all duration-200 rounded-lg"
              size="icon"
        ...
        <Button
          onClick={() => setSidebarOpen(true)}
              className="lg:hidden mr-3 bg-teal-600 hover:bg-teal-700 text-white shadow-sm"
          size="icon"
        >
          <Menu className="h-5 w-5" />
        </Button>

            {/* Logo Desktop - hanya logo, lebih besar dan tanpa border/shadow */}
